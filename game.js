...
class playGame extends Phaser.Scene{
    ...
    preload(){
        this.load.spritesheet("gems", "assets/sprites/donuts.png", {
            frameWidth: gameOptions.gemSize,
            frameHeight: gameOptions.gemSize
        });
        this.load.image('background', 'assets/sprites/background.jpg');
        this.load.image('score', 'assets/sprites/bg-score.png');
        this.load.image('btn-play', 'assets/sprites/btn-play.png');
        this.load.image('logo', 'assets/sprites/donuts_logo.png');
        this.load.image('btn-sfx', 'assets/sprites/btn-sfx.png');
        this.load.image('hand', 'assets/sprites/hand.png');
        this.load.image('shadow', 'assets/sprites/big-shadow.png');
        this.load.image('donut', 'assets/sprites/donut.png');
        this.load.image('back', 'assets/sprites/back-button-hi.png');
        this.load.image('tutorial0', 'assets/sprites/tutorial3.jpg');
        this.load.image('tutorial1', 'assets/sprites/tutorial1.jpg');
        this.load.image('tutorial2', 'assets/sprites/tutorial2.jpg');
        this.load.audio('menu', 'audio/background.mp3');
        this.load.audio('kill', 'audio/kill.mp3');
        this.load.audio('select', 'audio/select-1.mp3');
    ...
    create(){
        ...
        this.btnMusic = this.add.image(950, 600, 'btn-sfx')
            .setInteractive()
            .on('pointerdown', () => this.updateMusic(!musicClick));
        this.music = this.sound.add('menu');
        let clickCount = false;
        this.clickButton = this.add.image(550, 400, 'btn-play')
            .setInteractive()
            .on('pointerdown', () => this.updateClickCountText(!clickCount));
        this.updateClickCountText(clickCount);
        this.updateMusic(musicClick);
        let helpClick = false;
        this.clickHelp = this.add.image(150, 600, 'hand')
            .setInteractive()
            .on('pointerdown', () => this.helpBoard(!helpClick));
        this.helpText = this.add.text(120, 600, 'Help', { 
            font: "35px 'Fredoka One', cursive",
            fill: "#000",
            align: "center" 
        });
    ...
    helpBoard(helpClick) {
        if(helpClick) {
            this.bgd = this.add.image(600, 400, 'background');
            this.howToPlay = this.add.text(400, 100, 'How To Play', { 
                font: "50px 'Fredoka One', cursive",
                fill: "#000",
                align: "center" 
            });
            this.instructions = this.add.text(50, 250, 'Swap 2 items to match 3 or more of the same items in a row or a column', { 
                font: "30px 'Fredoka One', cursive",
                fill: "#000",
                align: "center" 
            });
            let backClick = false;
            this.backBtn = this.add.sprite(550, 620, 'back').setScale(0.3)
                .setInteractive()
                .on('pointerdown', () => this.backToMenu(!backClick));
            this.tutorialZero = this.add.sprite(200, 400, 'tutorial0');
            this.arrow = this.add.text(410, 370, '->', { 
                font: "30px 'Fredoka One', cursive",
                fill: "#e74c3c",
                align: "center" 
            });
            this.tutorialOne = this.add.sprite(600, 400, 'tutorial1');
            this.arrowTwo = this.add.text(760, 370, '->', { 
            font: "30px 'Fredoka One', cursive",
            fill: "#e74c3c",
            align: "center"
            });
            this.tutorialTwo = this.add.sprite(950, 400, 'tutorial2');
           
    }
}

    backToMenu(backClick) {
        if(backClick) {
            this.bgd.destroy();
            this.howToPlay.destroy();
            this.instructions.destroy();
            this.backBtn.destroy();
            this.tutorialZero.destroy();
            this.tutorialOne.destroy();
            this.tutorialTwo.destroy();
            this.arrow.destroy();
            this.arrowTwo.destroy();
        }
    }

    drawField(){
        //Score
        this.add.sprite(900, 150, 'score');
        this.add.sprite(900, 530, 'shadow').setScale(0.5);
        this.add.sprite(890, 530, 'donut').setScale(0.5);
        this.score = this.add.text(860, 105, '0', { 
            font: "50px 'Fredoka One', cursive",
            fill: "#fff",
            align: "center" 
        });
        ...
        this.timer = this.add.text(730, 250, 'Time lefts: 60', { 
            font: "50px 'Fredoka One', cursive",
            fill: "#000",
            align: "center" 
        })
        ...
        for(let i = 0; i < gameOptions.fieldSize; i ++){
            ...
            for(let j = 0; j < gameOptions.fieldSize; j ++){
                ...
                this.donutsGroup = this.gemGroup.add(gem);
                do{
        ...
        this.gameTimer = this.time.addEvent({
            ...
            callback: function(){
                this.timeLeft --;
                this.timer.text = `Time lefts: ${this.timeLeft}`;
                ...
                if(colorToWatch != currentColor || j == gameOptions.fieldSize - 1){
                    if(colorStreak >= 3){
                        if(direction == HORIZONTAL){
                            ...
                            this.score.text = parseInt(this.score.text) + colorStreak;
